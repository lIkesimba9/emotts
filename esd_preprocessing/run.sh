...
[ -d "$OUTPUT_DIR/processed/esd/$language/mfa_inputs/" ] && rm -rf $OUTPUT_DIR/processed/esd/$language/mfa_inputs/
[ -d "$OUTPUT_DIR/processed/esd/$language/mfa_outputs/" ] && rm -rf $OUTPUT_DIR/processed/esd/$language/mfa_outputs/
...
LEXICON_PATH="models/ESD.h-arpabet-lexicon.txt"
MODELS_PATH="models/english.zip"
...
if [ ! -f "models/ESD.h-arpabet-lexicon.txt" ]; then
    echo "don't exist $LEXICON_PATH then lexicon download from http://www.openslr.org/resources/11/librispeech-lexicon.txt ."
...
mfa align -t ./temp --clean -j 4 $OUTPUT_DIR/processed/esd/$language/mfa_inputs $LEXICON_PATH $MODELS_PATH $OUTPUT_DIR/processed/esd/$language/mfa_outputs
...
[ -d "$OUTPUT_DIR/processed/esd/$language/fastspeech2/" ] && rm -rf $OUTPUT_DIR/processed/esd/$language/fastspeech2/
